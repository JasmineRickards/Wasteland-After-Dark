...
/datum/component/construction/unordered/mecha_chassis/mw112mdd
	result = /datum/component/construction/mecha/mw112mdd
	steps = list(
...
/datum/component/construction/mecha/mw112mdd
	result = /obj/mecha/combat/mw112mdd
	base_icon = "mw112mdd"
	steps = list(
		//1
		list(
		...
		//5
		list(
			"key" = /obj/item/circuitboard/mecha/mw112mdd/main,
			"action" = ITEM_DELETE,
			"back_key" = TOOL_SCREWDRIVER,
			"desc" = "The wiring is adjusted."
		),
		...
		//7
		list(
			"key" = /obj/item/circuitboard/mecha/mw112mdd/peripherals,
			"action" = ITEM_DELETE,
			"back_key" = TOOL_SCREWDRIVER,
			"desc" = "Central control module is secured."
		),
		...
		//9
		list(
			"key" = /obj/item/circuitboard/mecha/mw112mdd/targeting,
			"action" = ITEM_DELETE,
			"back_key" = TOOL_SCREWDRIVER,
			"desc" = "Peripherals control module is secured."
		),
		...
		//20
		list(
			"key" = /obj/item/mecha_parts/part/mw112mdd_armour,
			"action" = ITEM_DELETE,
			"back_key" = TOOL_WELDER,
			"desc" = "Internal armor is welded."
		),
...
/datum/component/construction/mecha/mw112mdd/custom_action(obj/item/I, mob/living/user, diff)
	if(!..())
		return FALSE
	...
	switch(index)
		...
		if(21)
			if(diff==FORWARD)
				user.visible_message("[user] secures mw112mdd Armor Plates.", "<span class='notice'>You secure mw112mdd Armor Plates.</span>")
			else
				user.visible_message("[user] pries mw112mdd Armor Plates from [parent].", "<span class='notice'>You pry mw112mdd Armor Plates from [parent].</span>")
		if(22)
			if(diff==FORWARD)
				user.visible_message("[user] welds mw112mdd Armor Plates to [parent].", "<span class='notice'>You weld mw112mdd Armor Plates to [parent].</span>")
			else
				user.visible_message("[user] unfastens mw112mdd Armor Plates.", "<span class='notice'>You unfasten mw112mdd Armor Plates.</span>")
	return TRUE
