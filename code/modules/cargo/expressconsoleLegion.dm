...
/obj/machinery/computer/cargo/express/legion
	name = "express supply console"
	desc = "This console allows the user to purchase a package \
		with 1/40th of the delivery time: made possible by legion trebuchet links within the region, cushioning not included.\".\
		All sales are near instantaneous - please choose carefully"
	icon_screen = "supply_express"
	circuit = /obj/item/circuitboard/computer/cargo/express/legion
			...
			if(!(obj_flags & EMAGGED))
				if(SO.pack.cost <= points_to_check)
					...
					if (istype(beacon) && usingBeacon)//prioritize beacons over landing in cargobay
						...
						landingzone = GLOB.areas_by_type[/area/quartermaster/storage/legion]
						if (!landingzone)
